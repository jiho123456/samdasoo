...
for uid, un, ur in cur.fetchall():
    ...
    if st.session_state.role == "제작자":
        with col3.expander("킥하기"):
            reason = st.text_input("사유", key=f"kick_reason_{uid}")
            if st.button("강제 탈퇴", key=f"kick_{uid}"):
                cur.execute("""
                    INSERT INTO kicked_users(username, reason)
                    VALUES(%s, %s)
                    ON CONFLICT(username) DO UPDATE
                      SET reason=EXCLUDED.reason, kicked_at=NOW();
                """, (un, reason))
                cur.execute("DELETE FROM users WHERE username=%s", (un,))
                conn.commit()
                st.success(f"{un}님을 강제 탈퇴(킥)했습니다.\n사유: {reason}")
                st.experimental_rerun()
    else:
        col3.write("권한 없음")
    st.markdown("---")
...
cur.execute("SELECT id, post_id, username, comment, timestamp FROM blog_comments ORDER BY id DESC")
...
cur.execute("SELECT id, title, created_by, timestamp FROM quizzes ORDER BY id DESC")
...
for cid, cn, cd in cur.fetchall():
    ...
    if st.session_state.role in ["제작자","관리자"]:
        if col2.button("삭제", key=f"delclub_{cid}"):
            ...
            conn.commit()
            st.success("동아리와 관련 데이터를 모두 삭제했습니다.")
            st.experimental_rerun()
    st.markdown("---")
...
cur.execute("SELECT id, club_id, username, message, timestamp FROM club_chats ORDER BY id DESC")
...
cur.execute("SELECT id, club_id, username, file_path, upload_time FROM club_media ORDER BY id DESC")
...
# (필요시 10) kicked_users 관리 등 추가 가능
